...
@Component({
  selector: 'app-electricity-update',
  templateUrl: './electricity-update.component.html',
  styleUrls: ['./electricity-update.component.scss']
})
export class ElectricityUpdateComponent implements OnInit {
  ...
  update(){
    ...
    this.http.put('http://localhost:8000/expenses/update/' + this.toUpdateData.id , formData).subscribe(
      (result: any)  => {
        ...
        this.router.navigate(['secure/expenses-report/electricity-bill'])
      },
      error => {
        console.log('error')
        console.log(error);
