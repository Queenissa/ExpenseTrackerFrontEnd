...
@Component({
  selector: 'app-phone-bill-update',
  templateUrl: './phone-bill-update.component.html',
  styleUrls: ['./phone-bill-update.component.scss']
})
export class PhoneBillUpdateComponent implements OnInit {
  ...
  update(){
    ...
    this.http.put('http://localhost:8000/expenses/update/' + this.toUpdateData.id , formData).subscribe(
      (result: any)  => {
        ...
        this.router.navigate(['secure/expenses-report/phone-bill'])
      },
      error => {
        console.log('error')
        console.log(error);
